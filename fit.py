...
def fit(args, network, data_loader, **kwargs):
    ...
    # set gradient compression : none or 2bits
    if args.gc_type != 'none':
        kv.set_gradient_compression({'type': args.gc_type,
                                    'threshold': args.gc_threshold})
    # profile server actions into a file with name like rank1_ followed by this suffix 
    # during distributed training
    ...
    # math.ceil(int(args.num_examples / kv.num_workers) / args.batch_size)
    ...
    #if args.test_io:
    #    # test reading speed without training
    #    tic = time.time()
    ...
    # disp_batches : show progress for every n batches
	# speed = batch_size * disp_batches / (time1-time2) (samples/second)
